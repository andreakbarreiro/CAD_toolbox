function [assembly]=Main_assemblies_detection(spM,MaxLags,BinSizes,...
  Exp_Thres, ref_lag,alph,Dc,No_th,O_th,bytelimit)
...
%
%  2020 Modifications: Truong, Barreiro
%
...
if nargin<8 || isempty(No_th), No_th=0; end      % no limitation on the number of assembly occurrences
if nargin<9 || isempty(O_th), O_th=Inf; end     % no limitation on the assembly order (=number of elements in the assembly)
if nargin<10 || isempty(bytelimit), bytelimit=Inf; end     % no limitation on assembly dimension
